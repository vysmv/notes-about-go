...
Во многих языках (особенно компилируемых, как C, Go, Rust) runtime — это небольшая библиотека или слой, который компилятор добавляет в программу, чтобы управлять этими процессами. Например, даже если ты компилируешь `.go` или `.c` файл в бинарник, там будет присутствовать runtime (иногда минимальный, иногда довольно сложный).
...
Go-бинарник — это такой же полноправный процесс, как любой другой, написанный на C, Rust или Assembler.
